...
def analyze_with_sanppy(extension, input_folder):
            ...
            with open(os.path.join(input_folder, file)) as file:
                ...
                for line in file.readlines():
                    ...
                    if not graph.IsNode(n2):
                        graph.AddNode(n2)
                    graph.AddEdge(n1, n2)
    print("Nodes:", snap.CntNonZNodes(graph))
    print("Edges:", snap.CntUniqDirEdges(graph))
    print("Connected:", snap.IsConnected(graph))
    print("Average degree:", snap.CntNonZNodes(graph) / snap.CntUniqDirEdges(graph))
    ...
    print("Average shortest-path:", sp)
    print("Diameter:", diam)
    print("Effective diameter:", ef_diam_l, "-", ef_diam_h)
...
def analyze_with_networkx(extension, input_folder):
    ...
    print("Connected:", nx.is_connected(undir_graph))
